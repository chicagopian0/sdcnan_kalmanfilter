...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      double rho = measurement_pack.raw_measurements_[0]; // 长度
      double theta = measurement_pack.raw_measurements_[1]; // 角度
      double rho_dot = measurement_pack.raw_measurements_[2]; // 角速度
      ...
      cout << "First data from laser: " << endl;
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
    // TODO: Radar updates
    ekf_.H_ = tools.CalculateJacobian(ekf_.x);
    ekf_.R_ = R_radar_;
